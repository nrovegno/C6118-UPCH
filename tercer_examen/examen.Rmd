...
__Tabla 1. Índices del Fenómeno El Niño durante 1991__ _(valores en negritas corresponden a condiciones cálidas)_
...
En ese contexto se ha hecho un análisis de las bases de datos puestas a disposición en distintas escalas espacio-temporales, haciendo la acotación que el que suscribe desconoce la fuente de dicha información. Además se ha seguido la metodología descrita para hallar el ICEN utilizando los datos proporcionados.
...
Además se calculó el valor del ICEN con los valores de TSM provistos siguiendo la metodología dispuesta por el Comité ENFEN. Se calcularon los promedios mensuales para la región El Niño 1+2 (90°W -80°W, 10°S-0) para todo el periodo y luego se calculó la anomalía mensual de la media corrida de 3 meses de 1991 frente a a la media corrida del promedio mensual de todo el periodo.
...
#función anomalías
anho_1991=c(385:396)
Anom_1991 = function (var_nc, var_nom, var_lab, num){
...
par(oma=c(0,0,0,2))
plot(1:12, apply(sst_var[,,anho_1991],
    MARGIN = 3, FUN = mean, na.rm = TRUE), type = "l", 
    lwd=2, col="green", ylim = c(15,30), ylab = "TSM (°C)",
    xlab="", sub ="Figura 2. TSM (max, mean & min) durante el año 1991", axes = FALSE, las = 1)
axis(2, at = axTicks(2), las = 1)
axis(1, at = axTicks(1), labels = month.abb[seq(from = 2, to = 12, by = 2)])
lines(1:12, apply(sst_var[,,anho_1991], MARGIN = 3, FUN = min, na.rm = TRUE),
      col="blue")
lines(1:12, apply(sst_var[,,anho_1991], MARGIN = 3, FUN = max, na.rm = TRUE),
      col="red")
...
```{r grafico_mensual, echo = FALSE, fig.height=5, fig.width=5, fig.align="center"}
par(oma=c(0,0,1,2))
plot(1:12, apply(sst_var[,,anho_1991],
    MARGIN = 3, FUN = mean, na.rm = TRUE), type = "l", 
    lwd=2, col="green", ylim = c(15,30), ylab = "TSM (°C)",
